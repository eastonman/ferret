...
struct BranchHistoryFootprint : Benchmark {
  ...
  //
  // Ownership invariant: the JIT'd kernel bakes flat_.data() as a raw
  // pointer (SLJIT_IMM) inside emitted code. flat_ must not be
  // reassigned — which would invalidate that pointer — while any kernel
  // generated by the preceding emit_kernel call is still alive. The
  // framework destroys each JittedKernel before calling emit_kernel again,
  // so the invariant holds; take care when changing the call sequence.
